...
h = figure;
scale = 1;
h.Position = [10 50 scale*1500 scale*700];
hold on
...
% Sensor error
x_patch = [t, flip(t)];
y_patch = [theta_err+repeatability_error+sensor_error, flip(theta_err-repeatability_error-sensor_error)];
patch(x_patch,y_patch,'yellow','FaceAlpha',1)

% Repeatability error
x_patch = [t, flip(t)];
y_patch = [theta_err+repeatability_error, flip(theta_err-repeatability_error)];
patch(x_patch,y_patch,'cyan','FaceAlpha',1)

% Plot the line
plot(t,theta_err,'k','Linewidth', 2)
